...
class Experiment:
    ...
    def beginExperimentAndCollectMetaData(self):
        ...
        for i in range(self.numTrials):
            ...
            if i % 2 == 0:
                ...
                cv2.waitKey(self.trialLength * 1000)
                cv2.destroyAllWindows()
            # trialEndTime.strftime("%d-%b-%Y %H:%M:%S.%f")
    ...
    def runExperiment(self):
        ...
        self.experimentProcess = Thread(
            target=self.beginExperimentAndCollectMetaData)
        print("1")
    ...
    def splitRawDataIntoTrials(self, df):
        ...
        for i in range(len(self.experimentMetaData)):
            ...
            trialStartTimeValMask = df["timestamp"] == float(
                trial["trialStartTime"])
            trialEndTimeValMask = df["timestamp"] == float(
                trial["trialEndTime"])
