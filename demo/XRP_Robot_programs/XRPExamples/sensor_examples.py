...
def line_track():
    ...
    KP = 0.6
    while True:
        # You always want to take the difference of the sensors because the raw
        # value isn't always consistent.
...
def imu_test():
    while True:
        print(
            f"Pitch: {
                imu.get_pitch()}, Heading: {
                imu.get_heading()}, Roll: {
                imu.get_yaw()}, Accelerometer output: {
                    imu.get_acc_rates()}")
        time.sleep(0.1)
