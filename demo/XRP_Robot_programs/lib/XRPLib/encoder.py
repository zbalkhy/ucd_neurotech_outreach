...
class Encoder:
    ...
    def _encoder():
        ...
        # There is no explicite increment intruction, but X can be
        label("decr")
        # decremented in the jump instruction. So we use that and jump
        jmp(x_dec, "decr_nop")
        # to the next instruction, which is equivalent to just decrementing
        label("decr_nop")
        jmp("read")

        # There is no explicite increment intruction, but X can be
        label("incr")
        # decremented in the jump instruction. So we invert X, decrement,
        mov(x, invert(x))
        # then invert again - this is equivalent to incrementing.
        jmp(x_dec, "incr_nop")
        label("incr_nop")
        mov(x, invert(x))
        jmp("read")
        ...
        # For some reason, weird behavior happens if the instruction memory
        # isn't full
        jmp("read")
        jmp("read")
        jmp("read")
