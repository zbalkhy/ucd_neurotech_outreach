...
async def _handle_request(reader, writer):
    ...
    writer.write(
        f"HTTP/1.1 {response.status} {status_message}\r\n".encode("ascii"))
    ...
    logging.info(
        f"> {
            request.method} {
            request.path} ({
                response.status} {status_message}) [{processing_time}ms]")
...
def add_route(path, handler, methods=["GET"]):
    ...
    _routes = sorted(_routes, key=lambda route: len(
        route.path_parts), reverse=True)
