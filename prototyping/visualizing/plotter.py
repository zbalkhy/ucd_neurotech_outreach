...
class Plotter():
    ...
    def read_chunk(self):
        ...
        if new_chunk.size >= self.window_size:
            # if there is more data in the chunk than the window allows, just
            # take the most recent data
            self.current_chunk = new_chunk[new_chunk.size - self.window_size:]
        elif new_chunk.size < self.window_size:
            # if there isn't enough data to fill a new chunk, then assume we
            # have continuous data and tack it on to the end
    ...
    def filter_chunk(self):
        ...
        b, a = signal.butter(
            6, [8 / self.sampleRate * 2, 13 / self.sampleRate * 2], btype='bandpass')
        self.current_chunk = signal.filtfilt(b, a, self.current_chunk) / max(
            abs(signal.filtfilt(b, a, self.current_chunk))) * max(abs(self.current_chunk))
    ...
    def plot(self):
        ...
        fft_min = np.min(
            fft[np.where((freqs >= fft_xlims[0]) & (freqs <= fft_xlims[1]))])
        fft_max = np.max(
            fft[np.where((freqs >= fft_xlims[0]) & (freqs <= fft_xlims[1]))])
        ...
        line1, = ax1.plot(x, self.current_chunk, 'r-')
