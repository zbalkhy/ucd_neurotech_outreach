...
class NearestMean():
    ...
    def fit(self, features: np.ndarray, labels: np.ndarray):
        ...
        for x in self.unique_labels:
            ...
            #the weird things is to make sure labels are independent of their idx
            self.means.append(features[:, (indices==sum(np.where(self.unique_labels==x)))].mean(axis=1))
            self.stds.append(features[:, (indices==sum(np.where(self.unique_labels==x)))].std(axis=1))
    
    def predict(self, features: np.ndarray):
        distance = []
        for mean_var, std_var in zip(self.means, self.stds):
            #for each label (corresponding to index in list)
            #determine euclidian distance for each feature (normalized to standard deviation)
            distance.append(np.linalg.norm((features.T-mean_var)/std_var, axis=1))
