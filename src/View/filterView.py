...
class filterView(EventClass):
    def __init__(self, frame: tk.Frame, view_model: filterViewModel):
        ...
        self.filter_remove_button.pack(
            pady=10, padx=10, anchor="e", side=RIGHT)

        # create filter stream button
        self.create_filter_stream_button = tk.Button(
            self.filter_mod_frame,
            text="Make Filter",
            command=self.create_filter_stream,
            width=6,
            height=1,
            bg='yellow')
        self.create_filter_stream_button.pack(
            pady=10, padx=10, anchor="e", side=LEFT)

        # create a filter naming label
        self.filter_name_label = tk.Label(self.filter_mod_frame,
                                          text="Name:",
                                          anchor='e')
        self.filter_name_label.pack(
            pady=10, padx=(10, 0), anchor="e", side=LEFT)
    ...
    def update_filter_frame(self):
        ...
        self.filter_list_scrollbar = tk.Scrollbar(
        ...
        self.filter_list_canvas.configure(
            yscrollcommand=self.filter_list_scrollbar.set)
        ...
        # note, I know I can do this more efficiently but it's like 2am and
        # tkinter is kicking my butt -Andy

        # creates objects to interact with but holds actual value in a
        # stringvar object to keep saved
        label1 = tk.Label(self.scrollable_frame,
                          text='Filter Type:', anchor="w")
        ...
        entry1 = OptionMenu(self.scrollable_frame,
                            self.filter_boxes['filter'][0], *filter_types)
        ...
        entry2 = tk.Entry(self.scrollable_frame, width=2,
                          textvariable=self.filter_boxes['order'][0])
        ...
        entry3 = tk.Entry(self.scrollable_frame, width=7,
                          textvariable=self.filter_boxes['frequency'][0])
    ...
    def create_dropdown(self):
        ...
        self.dropdown = OptionMenu(
            master, self.reference_stream, *self.stream_names)
    ...
    def create_filter_stream(self):
        ...
        self.view_model.add_filter(self.filter_name_entry.get(
        ), filter_hold.lower(), order_hold, frequency_hold_float)

        self.view_model.create_filter_stream(
            self.filter_name_entry.get(), self.reference_stream.get())
